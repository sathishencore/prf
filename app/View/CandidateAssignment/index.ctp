...
<div class="bodyconrarea">
	<div class="content" id="createUserSection" style="display:;">
		<div class="content-mid">
									...
									<table width="94%" border="0" cellspacing="0" cellpadding="4"
										...
										<tr>
											...
											<td><label id="ProjectStartDate"><?php if(!empty($positionData)){ 
												echo $this->Time->format(Configure::Read("Date.Format"),$positionData['Project']['StartDate']);
											}?> </label>
											</td>
											<td>Status</td>
											<td>:</td>
										...
										<tr>
											...
											<td><label id="ProjectEndDate"><?php if(!empty($positionData)){ 
												echo $this->Time->format(Configure::Read("Date.Format"),$positionData['Project']['EndDate']);
											}?> </label>
											</td>
											<td>Billing Type</td>
											<td>:</td>
										...
										<tr>
											...
											<td>Client Project Manager</td>
											<td>:</td>
											<td><label id="ProjectManager"><?php if(!empty($positionData)){ 
												echo $clientContactList[$positionData['Project']['ProjectManagerID']];
											}?> </label>
											</td>
										</tr>
										<tr>
											<td rowspan="2">Primary Skills</td>
											<td rowspan="2">:</td>
											<td rowspan="2" width="28%"><label id="PrimarySkills"><?php if(!empty($positionData)){ 
												echo $positionData['Position']['EssentialSkills'];
											}?> </label>
											</td>
											<td rowspan="2">Secondary Skills</td>
											<td rowspan="2">:</td>
											<td rowspan="2"><label id="SecondarySkills"><?php if(!empty($positionData)){ 
												echo $positionData['Position']['OtherSkills'];
											}?> </label>
											</td>
											<td>Rate</td>
											<td>:</td>
											<td><label id="Rate"><?php if(!empty($positionData)){ 
												echo $positionData['Position']['BillingRate'];
											}?> </label>
											</td>
										</tr>
										<tr>
											...
											<td>:</td>
											<td><label id="Rate"><?php if(!empty($positionData)){
												if($positionData['Position']['UnitOfMeasure'] == "Fixed fee") 
													$positionData['Position']['UnitOfMeasure'] = "Hourly for Fixed fee";
												else if($positionData['Position']['UnitOfMeasure'] == "Milestone based")
													$positionData['Position']['UnitOfMeasure'] = "Hourly for Milestone based";
												
												echo $positionData['Position']['UnitOfMeasure'];
											}?> </label>
											</td>
										</tr>
										<tr>
											...
											<td>:</td>
											<td><?php if(!empty($positionData)){ 
												if(count($ConfirmedStatus) > 0){?><a href="javascript:"
												class="assignment_status" data-val="Confirmed"> <?php echo count($ConfirmedStatus);
}else{ echo '0';
}
}?>
										...
										<tr>
											...
											<td>:</td>
											<td><?php if(!empty($positionData)){ 
												if(count($AssignedStatus) > 0){?><a href="javascript:"
												class="assignment_status" data-val="Assigned"> <?php echo count($AssignedStatus);
}else{ echo '0';
}
											}?>
											</a>
											...
											<td>:</td>
											<td><?php if(!empty($positionData)){ 
												if(count($SubmittedStatus) > 0){?><a href="javascript:"
												class="assignment_status" data-val="Submitted"> <?php echo count($SubmittedStatus);
}else{ echo '0';
}
											}?>
											</a>
											...
											<td>:</td>
											<td><?php if(!empty($positionData)){ 
												if(count($InterviewScheduledStatus) > 0){?><a
												href="javascript:" class="assignment_status"
												data-val="Interview Scheduled"> <?php echo count($InterviewScheduledStatus);
}else{ echo '0';
}
											}?>
											</a>
										...
										<tr>
											...
											<td>:</td>
											<td><?php if(!empty($positionData)){ 
												if(count($InterviewDoneStatus) > 0){?><a href="javascript:"
												class="assignment_status" data-val="Interview Done"> <?php echo count($InterviewDoneStatus);
}else{ echo '0';
}
											}?>
											</a>
											...
											<td>:</td>
											<td><?php if(!empty($positionData)){ 
												if(count($SelectedStatus) > 0){?><a href="javascript:"
												class="assignment_status" data-val="Selected"> <?php echo count($SelectedStatus);
}else{ echo '0';
}
											}?>
											</a>
											...
											<td>:</td>
											<td><?php if(!empty($positionData)){ 
												if(count($RejectedStatus) > 0){?><a href="javascript:"
												class="assignment_status" data-val="Rejected"> <?php echo count($RejectedStatus);
}else{ echo '0';
}
											}?>
											</a>
										...
										<tr>
											...
											<td colspan="6"><?php 
											echo $this->Html->link(
											    $this->Html->image("cancel-inner-but.png", array("alt" => "Cancel","style"=>"float:right;")),
											    "/CandidateAssignment/",
											    array('escape' => false)
											);?>
											</td>
			...
			<?php if($this->Session->read("roleId") == 5){ 
				echo $this->element("unassignedPositions");
}?>
		</div>
	</div>
</div>
...
$("#CandidateAssignmentClientID").change(function(){
	 ...
	 $.ajax({
		 ...
		 data: { CandidateAssignmentClientId:$("#CandidateAssignmentClientID").val() }
		 }).done(function( msg ) {
...
$("#CandidateAssignmentProjectID").change(function(){
	 ...
	 $.ajax({
		 ...
		 data: { CandidateAssignmentProjectId:$("#CandidateAssignmentProjectID").val(),CandidateAssignmentClientId:$("#CandidateAssignmentClientID").val() }
		 }).done(function( msg ) {
